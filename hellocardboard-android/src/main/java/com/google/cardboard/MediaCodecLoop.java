 ...
 * This uses various features first available in Android "Jellybean" 4.3 (API
 * 18). There is
 * no equivalent functionality in previous releases. (You can send the Camera
 * preview to a
 * byte buffer with a fully-specified format, but MediaCodec encoders want
 * different input
...
public class MediaCodecLoop {
    ...
    private static final String TAG = "MediaCodecLoop";
    ...
    // private CodecInputSurface mOutputSurface;
    ...
    public void encodeCameraToMpeg() {
        ...
        try {
            ...
            Log.e(TAG, "encodeCameraToMpeg() failed.", e);
        } finally {
    ...
    /**
     * Attempts to find a preview size that matches the provided width and height
     * (which
     * specify the dimensions of the encoded video). If it fails to find a match it
     * just
     ...
     * Initializes
     * mEncoder, mMuxer, mInputSurface, mEncoderBufferInfo, mTrackIndex, and
     * mMuxerStarted.
     */
    private void prepareEncoder(int width, int height, int bitRate) {
        ...
        try {
            ...
            throw new RuntimeException("failed to create encoder", ioe);
        }
        ...
        try {
            mMuxer = new MediaMuxer(fFile.getAbsolutePath(), MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
        } catch (IOException ioe) {
    ...
    private void feedDecoder(ByteBuffer encodedData) {
        ...
        VideoChunks decInputData = new VideoChunks();
        decInputData.setMediaFormat(mEncoder.getOutputFormat());
        decInputData.addChunk(encodedData, mDecoderBufferInfo.flags, mDecoderBufferInfo.presentationTimeUs);
        ...
        while (!outputDone) {
            if (!inputDone) {
                ...
                if (inputBufIndex >= 0) {
                    if (inputChunk == decInputData.getNumChunks()) {
                        ...
                        inputBuf.clear();
                        decInputData.getChunkData(inputChunk, inputBuf);
                        mDecoder.queueInputBuffer(inputBufIndex, 0, inputBuf.position(),
                                decInputData.getChunkTime(inputChunk), decInputData.getChunkFlags(inputChunk));
                        if (VERBOSE) {
                            Log.d(TAG, "submitted frame " + inputChunk + " to dec, size=" +
                                    inputBuf.position() + " flags=" + decInputData.getChunkFlags(inputChunk));
                        }
                        inputChunk++;
                    }
                }
            ...
            while (decoderOutputAvailable) {
                ...
                if (!decoderDone) {
                    //// Need to kill decoderDone
                    int decoderStatus = mDecoder.dequeueOutputBuffer(mDecoderBufferInfo, TIMEOUT_USEC);
                    if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                        ...
                        throw new RuntimeException(
                                "unexpected result from decoder.dequeueOutputBuffer: " + decoderStatus, null);
                    } else { // decoderStatus >= 0
                        if (VERBOSE)
                            Log.d(TAG, "surface decoder given buffer "
                                    + decoderStatus + " (size=" + mDecoderBufferInfo.size + ")");

                        ByteBuffer[] decoderOutputBuffers = mDecoder.getOutputBuffers();
                        ByteBuffer decodedDataDisplay = decoderOutputBuffers[decoderStatus];

                        try {
                            FileOutputStream out = new FileOutputStream("video.mp4");
                            out.write(decodedDataDisplay.array());
                            out.close();
                        } catch (Exception e) {
                        ...
                        if ((mDecoderBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                            ...
                            Log.d(TAG, "signaling output EOS");
                            outputDone = true;
                        }
                    }
                }
    ...
    /**
     * The elementary stream coming out of the "video/avc" encoder needs to be fed
     * back into
     * the decoder one chunk at a time. If we just wrote the data to a file, we
     * would lose
     * the information about chunk boundaries. This class stores the encoded data in
     * memory,
     ...
     * MediaCodec.createInputSurface(), and uses
     * that to create an EGL window surface. Calls to eglSwapBuffers() cause a frame
     * of data to
    ...
    /**
     * Manages a SurfaceTexture. Creates SurfaceTexture and TextureRender objects,
     * and provides
    ...
    private static class SurfaceTextureManager
        ...
        public SurfaceTextureManager() {
            ...
            // This doesn't work if this object is created on the thread that CTS started
            // for
            // these test cases.
            //
    ...
    private static class STextureRender {
        ...
        public void drawFrame(SurfaceTexture st) {
            ...
            // IMPORTANT: on some devices, if you are sharing the external texture between
            // two
            // contexts, one context may not see updates to the texture unless you un-bind
            // and
