...
struct DeviceLinearAccelerationSensor::SensorInfo {
...
DeviceLinearAccelerationSensor::DeviceLinearAccelerationSensor()
...
void DeviceLinearAccelerationSensor::PollForSensorData(
    int timeout_ms, std::vector<LinearAccelerationData>* results) const {
  results->clear();
  ASensorEvent event;
...
bool DeviceLinearAccelerationSensor::Start() {
  if (!sensor_info_->reader) {
    CARDBOARD_LOGE("Could not start linear acceleration sensor");
    return false;
  }
  return sensor_info_->reader->Start();
}

void DeviceLinearAccelerationSensor::Stop() {
  if (!sensor_info_->reader) {
    return;
  }
